...
> 2. For every task: use `subagent_type: "implementation-executor"` in the Agent call — never the default agent.
...
Replace `<plugin-root>` with the absolute path to this plugin's root directory (the directory containing `scripts/task-sidecar.sh`).
...
When launching an `implementation-executor` agent for a task, you **must** use `subagent_type: "implementation-executor"` in the Agent tool call — never use the default general-purpose agent. Using the wrong agent type means the coding standards and quality gates defined in the `implementation-executor` agent definition will not be applied.
...
> **Critical**: The Agent tool call must always include `subagent_type: "implementation-executor"`. Example skeleton:
>
> ```
> Agent({
...
Note in your execution report: "Resuming from checkpoint — TASK-1, TASK-2 already completed. Starting from TASK-3." Then immediately proceed to **Execution Workflow** below, skipping tasks in `completed`/`failed`/`blocked`. Do NOT stop or wait for user input — every task must be delegated to an `implementation-executor` subagent via the Agent tool (`subagent_type: "implementation-executor"`), exactly as described in the Execution Workflow.
...
1. **Parse the plan**:
   - Run `bash <plugin-root>/scripts/task-sidecar.sh list <plan-dir>/compiled/manifest.json` to enumerate all task IDs
...
3. **Execute tasks sequentially — one at a time**:
   ...
   - Complete each task fully before starting the next:
     1. Launch `implementation-executor` agent using the **Agent Prompt Template** above. Set `subagent_type: "implementation-executor"`, pass the script path, plan path, task ID, and compiled script path (`<plan-dir>/compiled/<TASK-ID>.sh`). Do NOT copy task content into the prompt; the agent runs the compiled script directly.
   ...
   - Never batch tasks or launch multiple agents in parallel — strict sequential order preserves plan integrity
...
2. Execute TASK-1 with `implementation-executor` agent (`subagent_type: "implementation-executor"`):
...
3. Execute TASK-2 with `implementation-executor` agent (`subagent_type: "implementation-executor"`):
   - Create seed files
...
4. Execute TASK-3 with `implementation-executor` agent (`subagent_type: "implementation-executor"`):
